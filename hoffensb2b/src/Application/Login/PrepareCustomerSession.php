...
final class PrepareCustomerSession
{
    ...
    public function execute($customerId)
    {
        ...
        if ($this->cacheTtl > 0) {
            ...
            if (is_array($cached)) {
                try {
                    $this->validator->validate($cardCode, $cached);
                    $metrics['totalUseCaseMs'] = $this->elapsedMs($startedAt);
                    return LoginDecision::allowed($cached, true, $metrics, true);
                } catch (IntegrationException $exception) {
                    // Un caché incompleto o perteneciente a otro cardCode no habilita compras.
                    $metrics['cacheRejected'] = true;
                }
            }
        }
        ...
        try {
            ...
            $metrics['totalUseCaseMs'] = $this->elapsedMs($startedAt);
            return LoginDecision::allowed($snapshot, false, $metrics, true);
        } catch (IntegrationException $exception) {
            $metrics['totalUseCaseMs'] = $this->elapsedMs($startedAt);
